...
// Updated to return a full breakdown of the score
const calculateLandscapeScore = (board) => {
    ...
    // 2. MOUNTAINS (Stone)
    // Points = Height, BUT only if adjacent to another Mountain
    cells.forEach(cell => {
  ...
  if (view === "game" && gameState) {
      ...
      return (
        <div className="fixed inset-0 bg-slate-950 text-white flex flex-col overflow-hidden font-sans select-none">
            ...
            {showLogs && (
                <div className="fixed top-16 right-4 w-64 max-h-60 bg-slate-900/95 border border-slate-700 rounded-xl z-[155] overflow-y-auto p-2 shadow-2xl">
            ...
            <div className="h-40 bg-transparent absolute bottom-0 left-0 right-0 z-40 p-4 flex justify-between items-end pointer-events-none">
                <div className="flex gap-4 pointer-events-auto items-end w-full">
                    ...
                    {/* ANIMAL HAND AREA */}
                    <div className="flex gap-2 items-end ml-2 overflow-x-auto pb-1 max-w-[calc(100vw-160px)] no-scrollbar mask-gradient-right h-48">
                        {viewingPlayer.animals.map((card, i) => {
                             ...
                             return (
                                 <button 
                                    ...
                                    className={`relative w-28 h-40 bg-slate-900/90 border-2 rounded-xl flex flex-col shadow-xl shrink-0 backdrop-blur-md transition-all hover:-translate-y-2 text-left ${isSelected ? 'border-yellow-400 ring-2 ring-yellow-500/50 scale-105 z-10' : 'border-slate-600'} ${isComplete ? 'grayscale opacity-75' : ''}`}
                                 >
                                     ...
                                     <div className="p-2 flex-1 flex flex-col items-center justify-center">
                                        ...
                                        <div className="text-[9px] text-slate-400 text-center leading-tight mb-1 px-1 h-8 overflow-hidden">{def.desc}</div>
                                        <div className="mt-auto w-full pt-1">
                                            ...
                                            <div className="flex gap-1 justify-center">
                                                {Array.from({length: card.maxSlots}).map((_, i) => (
                                                    <div key={i} className={`h-2 w-full rounded-full border ${i < card.slotsFilled ? 'bg-green-500 border-green-400' : 'bg-slate-700 border-slate-600'}`}></div>
                                                ))}
                                            </div>
                                        </div>
                                     </div>
